...
export function useLockSync({ id, content, dataFromRedux, studyprogrammeKey, year }: UseLockSyncProps) {
  ...
  useEffect(() => {
    const gotTheLock = lockMap?.[id] && lockMap[id].uid === currentUser.uid
