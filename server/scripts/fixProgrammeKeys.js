...
const fixProgrammeKeys = async () => {
  try {
    logger.info('Fixing programmes')

    const answers = await db.answer.findAll()

    for (let answer of answers) {
      const isOld = oldProgrammes.find((prog) => prog.key === answer.programme)

      if (isOld) {
        const id = isOld.id
        const newKey = programmes.find((p) => p.id === id).key
        answer.programme = newKey
        await answer.save()
      }
    }
    logger.info('Programmes fixed')
  } catch (error) {
    logger.error(`Failed to fix programmes: ${error}`)
  }
}
module.exports = {
  fixProgrammeKeys,
